...
imgNave = new Image();
imgOvni = new Image();
imgOvni.src = "img/seis/ALIEN.png";
imgNave.src = "img/seis/FIN2.png";

// var audio = document.getElementById("dispara");
...
function iniciando() {
	...
	// canvas.style.backgroundImage = 'url("imagenes/espacio.gif")';
	...
	for (var i = 0; i < 5; i++) {
		for (var j = 0; j < 10; j++) {
			ovnis_array.push(new Enemigo(100 + 55 * j, 35 + 40 * i));
		}
	}
...
function nave(x) {
	...
	this.dibuja = function (x) {
		this.x = x;
		if (imgAni2 < 5) {
			ctx.drawImage(imgNave, 0, 0, 300, 350, this.x, this.y, 100, 100);
			imgAni2 = imgAni2 + 1;
			imgAni = imgAni + 1;
			checarBalas();
			//setInterval(checarBalas(),1000);
		} else if (imgAni2 < 10) {// 600, 0, 300, 350, this.x, this.y, 60,80);
			ctx.drawImage(imgNave, 300, 0, 300, 350, this.x, this.y, 100, 100);
			imgAni2 = imgAni2 + 1;
			imgAni = imgAni + 1;
		} else {
			ctx.drawImage(imgNave, 300, 0, 300, 350, this.x, this.y, 100, 100);
...
function gameOver() {
	...
	if (enemigosVivos == 0) {
		mensaje("GANASTE");
		var img = new Image();
		img.src = "img/seis/marcianito_family.png";
		img.onload = function () {

			ctx.drawImage(img, 250, 200, 10, 10, 10, 10, 10);
		}
	} else {
		mensaje("GAME OVER");
		var img = new Image();
		img.src = "img/seis/marcianito_llora.png";
		img.onload = function () {
			ctx.drawImage(img, 250, 200);
		}
		// canvas.style.backgroundImage = 'url("imagenes/BACK.png")';
		canvas.style.backgroundSize = 'cover';
